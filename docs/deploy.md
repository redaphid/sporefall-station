...
> Access login page instead of the app — `302 →
> `<team>.cloudflareaccess.com`` for any signed-out phone, which is exactly the
> device you wanted to hand the preview to.
>
...
>
> **Use a separately-named Worker instead.** `wrangler.preview.jsonc` defines
> `sporefall-preview` — a different `name`, therefore a different script, which
> cannot resolve to `sporefall-station`. It carries its own custom hostname on a
> zone that is *not* Access-protected, so a signed-out phone can load it.
>
> ```bash
> pnpm run build
> pnpm exec wrangler deploy -c wrangler.preview.jsonc   # deploy the preview
> pnpm exec wrangler delete -c wrangler.preview.jsonc   # tear it down
> ```
>
> Nothing in that file reads or rewrites production's config, routes or hostname.
