...
> **Provenance — read before citing a number.** This is a **snapshot analysis**,
> measured against `c189558` and brought onto `main` on 2026-08-18 (at `4b62cb1`)
> so it is not lost in a scratch worktree. The measurements below were **not**
> re-run at `4b62cb1`, and `#40` has since changed client-side smoothing — treat
> every figure as "true at `c189558`" unless you re-measure. The appendix gives the
> commands.
>
> **What was re-verified at `4b62cb1`:** finding 3 (the client-side input-sequence
> u16 wrap) is **still open**. `netHost.ts:430` compares wrap-tolerantly
> (`cmd.seq > p.lastInputSeq || p.lastInputSeq - cmd.seq > 30000`), but
> `netClient.ts:435` still prunes with a plain `p.seq > this.lastAckedSeq` while
> `netClient.ts:474` increments an **unbounded** counter that the wire masks to u16
> (`messages.ts:294`). The 60-entry backlog cap is real (`netClient.ts:509`), so
> once the wire wraps, nothing prunes and `reconcile()` replays 60 inputs (2.0 s at
> 30 Hz) on every snapshot, forever. 65536 inputs at 30 Hz = **36.4 minutes** of
> continuous play. Still no test covers it.
