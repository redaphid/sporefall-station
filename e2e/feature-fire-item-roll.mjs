// FIRE-button arbitration headline (feat/fire-use-roll): one button resolves to
// use-item / fire-weapon / dodge-roll off the ACTIVE slot + availability.
//   • fire-uses-item : a wounded player holding a bandage presses FIRE → the
//                      bandage is USED (heals) and spent; NO bullet is fired.
//   • fire-rolls     : the active gun is EMPTY, so FIRE (nothing to shoot or use)
...
// 2) Fire an empty gun → dodge-roll through the bullet. Full hp, pistol at 0 ammo.
{
  ...
  ok =
    (await recordFeature({
      name: 'fire-rolls-when-empty',
      world: w,
      script: 'fireFallbackRoll',
      stills: [
        { tick: 5, label: '01-empty-gun-bullet-inbound' },
        { tick: 17, label: '02-fire-triggered-roll' },
        { tick: 24, label: '03-rolled-through' },
        { tick: 45, label: '04-unharmed' },
      ],
      readState,
      expect: (s) => [
        s.playerHp !== s.playerMax && `took damage — the fire→roll fallback did not save it (hp ${s.playerHp}/${s.playerMax})`,
        s.downed && 'player went down',
        s.bullets > 0 && `an empty gun fired a bullet (${s.bullets})`,
      ],
    })) && ok
}
