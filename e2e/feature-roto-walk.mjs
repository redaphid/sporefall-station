// feat/rotoscoped-walk — the swampspace vine-ranger's 8-frame rotoscoped walk
// cycles (scripts/assets/rotoscope/), shown in game in all 8 facings.
// Same stage + walk8 compass-circle script as feature-walk8.mjs, but rendered
// under ?theme=swampspace so the ranger animates through the new
// char.player.<dir>-walk-0..7 clips (west half mirrored by the engine).
// Post-run asserts pin that the sim really walked the circle.
...
// Same timeline as feature-walk8: 20 settle, then 8 × (14 move + 70 hold).
const stills = [
...
await recordFeature({
  name: process.env.ROTO_NAME || 'roto-walk-swampspace',
  world: stage(),
  script: 'walk8',
  ...
  expect: (s) => [
    ...
    s.facing !== 0 && `post-script facing ${s.facing} ≠ 0 (did the walk8 script run?)`,
