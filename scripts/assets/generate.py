...
# ONE object, and specifically not a STACK of them. Measured on the first sweep
# with the fixed recipe: 6 of 10 cargo-crate seeds came back as a vertical PILE
# of crates (two stacked, a tower, a wall of shelving) -- far and away the
# dominant defect once the moss and the tree were closed off. NEG_BASE already
# said "several objects side by side, duplicate, two copies", and none of it bit,
# because the failure is not lateral repetition: crates are a thing the model has
# overwhelmingly seen STACKED, so it stacks them vertically and the side-by-side
# wording never touches that. Name the vertical arrangement, the same way `tree`
# and `dark ellipse under the object` had to be named. This is shared by all
# props, not just the crate -- a rack of lockers or a row of desks is the same
# error, and the sprite has to be ONE placeable object either way.
NEG_STACK = ("stack, stacked, stacked on top of each other, pile, heap, tower of boxes, "
             "two objects, three objects, multiple objects, group of objects, cluster, "
             "collection, set of containers, shelving unit, rack, storage rack, shelves, "
             "warehouse, one on top of another, second object behind, smaller copy beside it")
...
def jobs():
    ...
    for name, (path, subj, extra) in PROPS.items():
        out[f"prop.{name}"] = dict(cat="prop", path=path, px=PROP_PX,
                                   pos=f"{TRIGGER}, {subj}, {BG_OBJ}, exactly one single object "
                                       f"and nothing else in frame, nothing underneath it, "
                                   ...
                                   neg=f"{NEG_FIGURE}, {NEG_BASE}, {NEG_GROUND}, "
                                       f"{NEG_WRONG_READ}, {NEG_STACK}, {extra}")
