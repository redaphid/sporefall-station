...
//
//   npx tsx scripts/playtest.mts <state.json> new [--seed N] [--scenario NAME] [--floor F]
...
if (!file || !verb) {
  console.error('usage: playtest.mts <state.json> new [--seed N] [--scenario NAME] [--floor F] | <verb line>')
  process.exit(2)
}
...
if (verb === 'new') {
  ...
  const host = new HostSession(seed, { sample: emptyInput })
