 ...
 *    writes a plain sim value into the new World, exactly as the difficulty
 *    `mode` is chosen. From then on the rule is world state: it serializes with
 *    the save, replays with a recording, and reaches clients in GameStart. The
 *    sim and the protocol still never read a flag (the grep test still holds),
 *    a client's own setting has no effect on the host's run, and toggling
 *    mid-run changes nothing until the next run is built.
 *
