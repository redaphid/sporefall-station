...
const startPair = async (
  ...
  const bob = hub.addClient('Bob', 'thief', clientInput)
  await host.start()
...
describe('co-op client inventory (issue #57)', () => {
  ...
  it('gives multiple clients each THEIR own inventory, not each other\'s', async () => {
    ...
    const bob = hub.addClient('Bob', 'thief', makeInput().source)
  ...
  it('re-sends the full inventory to a rejoining client (ghost reclaim)', async () => {
    ...
    b.connect()
    await flush()
    b.send(encodeJson(MsgType.Hello, { v: PROTOCOL_VERSION, name: 'Bob', classId: 'thief', rejoin: { slot: welcome.slot, token: welcome.token } }))
    await flush()
    host.tick()
    await flush()
