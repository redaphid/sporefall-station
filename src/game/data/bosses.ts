 ...
 *
 * APPEND a row to add a boss. The two still on the design doc's list
 * (Mirefather, the Hollow Choir) are NOT here yet: a row here makes a boss
 * selectable by `pickBoss`, so registering one before its NPCS row and system
 * exist would spawn an archetype with no definition. Their wire indices are
 * already reserved in `net/protocol/messages.ts` — that list is append-only and
 * must be booked ahead; this one must not be.
 */
