...
export const applyStatus = (w: World, e: Entity, status: string, ticks: number): void => {
  ...
  // re-armed the clock on every hit. They immobilize just as totally as `frozen`
  // does, so the perma-lock the guard exists to prevent was wide open here: a
  // sledgehammer stuns for 20 ticks on a 28-tick swing (safe from one attacker),
  // but it is not archetype-locked — it sits in NPC_ARSENAL, so ordinary mobs
  // roll it, and two of them swinging out of phase reset the stun forever while
  // 26-damage blows keep landing. Alert escalation converging a floor on one
  // player is exactly that situation.
  //
