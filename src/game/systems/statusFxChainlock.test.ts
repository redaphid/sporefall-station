...
describe('immobilize anti-chain-lock', () => {
  ...
  // This test used to assert that stun/sleep created NO lockout tracker, which
  // documented the hole rather than a feature: those two immobilize as totally as
  // `frozen` but were a flat overwrite, so any two attackers landing hits closer
  // together than the duration reset the timer forever. A sledgehammer is 20
  // ticks on a 28-tick swing and is NOT archetype-locked — it is in NPC_ARSENAL,
  // so ordinary mobs roll it. They now ride the same guard, on their own tracks.
