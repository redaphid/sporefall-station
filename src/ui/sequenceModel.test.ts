...
describe('sequence strip model', () => {
  it('is hidden unless the run is sequenced and the weapon has mods', () => {
    expect(buildSequence(tech([m('frost')]), undefined, 0)).toBeNull()
    expect(buildSequence(tech([]), 'sequence', 0)).toBeNull()
    expect(buildSequence(undefined, 'sequence', 0)).toBeNull()
  })

  it("highlights the cast at the sim's stored castIndex: modifiers through the payload", () => {
    const model = buildSequence(tech([m('frost'), m('overload'), m('shock'), m('incendiary')], 1), 'sequence', 0)!
  ...
  it('marks entries past the weapon slots as stowed', () => {
    const model = buildSequence(tech([m('frost'), m('shock'), m('incendiary'), m('overload'), m('pierce')]), 'sequence', 0)!
  ...
  it('previews queued swaps without touching the stack', () => {
    ...
    const model = buildSequence(p, 'sequence', 0, (mods) => previewSwaps(mods, [{ a: 0, b: 2 }]))!
